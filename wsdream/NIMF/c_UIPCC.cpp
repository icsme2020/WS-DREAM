/********************************************************
 * UIPCC.cpp: C++ implements on PCC computations
    ...
    for (i = 0; i < numUser; i++) {
  	...
  	for (i = 0; i < numUser; i++) {
		...
		while (k < topK && it != sortedPccMap.end()) {
			int uid = it->first;
			double pccValue = it->second;
			topkSimUserDic[i].push_back(make_pair(uid, pccValue / sum));
			topkSimUserDicRecord[uid].push_back(make_pair(i, pccValue / sum));
			it++;
			k++;
		}
  	}
