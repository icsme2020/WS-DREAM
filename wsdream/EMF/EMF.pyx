...
#
cdef extern from "c_EMF.h":
    void c_EMF(double *removedData, int numUser, int numService, int dim, 
...
def predict(removedMatrix, para):  
    ...
    # Wrap up EMF.cpp
    c_EMF(<double *> (<np.ndarray[double, ndim=2, mode='c']> removedMatrix).data,
        numUser,
        numService,
        dim,
        lmda,
