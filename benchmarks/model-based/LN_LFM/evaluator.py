...
#======================================================#
def execute(matrix, wsInfoList, para):
        ...
        for den in para['density']: 
            for roundId in xrange(para['rounds']):
                pool.apply_async(executeOneSetting, 
                	(matrix, wsInfoList, den, roundId, para))
        pool.close()
        pool.join()
    else: # run on single processes
        for den in para['density']:
            for roundId in xrange(para['rounds']):
                executeOneSetting(matrix, wsInfoList, den, roundId, para)
...
#======================================================#
def executeOneSetting(matrix, wsInfoList, density, roundId, para):
    ...
    predictedMatrix = LN_LFM.predict(trainMatrix, wsInfoList, para) 
